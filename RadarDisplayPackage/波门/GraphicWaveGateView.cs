...
namespace RadarDisplayPackage
{
    class GraphicWaveGateView : IDisposable
    {
        ...
        public GraphicWaveGateView(WaveGate waveGate, OverViewDisplayer ovd)
        {
            ...
            if(waveGate.IsSemiAuto)
                waveGateBrush = canvas.CreateSolidColorBrush(GraphicTrackDisplayer.GetColorFFromRgb(255 ,255 ,0));   //黄色
            else
                waveGateBrush = canvas.CreateSolidColorBrush(GraphicTrackDisplayer.GetColorFFromRgb(245, 222, 179));   //淡黄色
            ...
            c.Az = waveGate.BeginAngle;
            c.ProjectedDis = waveGate.BeginDistance;
            innerLeft = coodinateSystem.CoordinateToPoint(c);

            c.Az = waveGate.EndAngle;
            c.ProjectedDis = waveGate.EndDistance;
        ...
        public bool Selected
        {
            get
            {
                return waveGate.Active;
            }

            set
            {
                waveGate.Active = value;
            }
        }
